...
namespace com.ihaiu
{
    public partial class VersionPushWindow 
    {
		...
		void Step_AddTag()
		{
			SettingConfig SettingConfig = SettingConfig.Load(gitServerEditor.gitSettingConfigPath);
            ...
            for(int i = 0; i < centerList.Count; i ++)
			{
				...
				if (item.gitToggle)
				{
					...
					SettingConfig.version.centerName = item.name;
                    SettingConfig.Save(gitServerEditor.gitSettingConfigPath);

                    string verinfoPath = gitServerEditor.GetGitVerinfoPath(item.name, true);
                    VersionInfo versionInfo = VersionInfo.Load(verinfoPath);
					versionInfo.version = SettingConfig.version.centerName;
					...
					tmp.WriteLine(string.Format("git commit -am \" 修改SettingConfig.centerName= {1}\" 2>&1 | tee -a {0}", Shell.txt_vertmp, item.name));
