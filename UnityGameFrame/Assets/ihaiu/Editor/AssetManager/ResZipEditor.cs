...
namespace com.ihaiu
{
    ...
    public class ResZipEditor 
    {
        ...
        {
            string listPath = AssetManagerSetting.EditorFilePath.AssetListFile;
            ...
            if (!File.Exists(AssetManagerSetting.EditorFilePath.AssetCollectGuide))
            {
                Debug.LogError("不存在:" + AssetManagerSetting.EditorFilePath.AssetCollectGuide);
                EditorWindow.mouseOverWindow.ShowNotification(new GUIContent("不存在:" + AssetManagerSetting.EditorFilePath.AssetCollectGuide));
                return;
            }
...
//            AssetCollect    collect = AssetCollect.LoadGuide();
            ...
            collect.OnLoadInternal(AssetManagerSetting.FileName.SettingConfig);
...
//            Image/Soldier/Soldier_30165";
            ...
            appAssetList.Save(AssetManagerSetting.EditorFilePath.AssetListApp);
            zipAssetList.Save(AssetManagerSetting.EditorFilePath.AssetListZip);
        }


        public void CopyToResZip()
        {
            string listPath = AssetManagerSetting.EditorFilePath.AssetListZip;
            if (!File.Exists(listPath))
            {
                GenerateAssetListZipAndApp();
            }
        ...
        {
            ZipHelper.ZipDirectory(AssetManagerSetting.EditorRoot.WorkspaceResZip, AssetManagerSetting.EditorFilePath.ResZip);
        }
        ...
        {
            string listPath = AssetManagerSetting.EditorFilePath.AssetListApp;
            if (!File.Exists(listPath))
            {
                GenerateAssetListZipAndApp();
            }
        ...
        private void DeleteApp(AssetFileList assetList)
        {
            ...
            deleteIgnoreDict.Add(AssetManagerSetting.FileName.SettingConfig, true);
