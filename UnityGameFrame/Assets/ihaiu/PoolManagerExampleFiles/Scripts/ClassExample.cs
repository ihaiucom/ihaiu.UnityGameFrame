...
{
    public class UnitDataAAA : ScriptableObject
    {
    ...
    public class ClassExample : MonoBehaviour {
        ...
        ScriptableObjectPool<UnitDataAAA> unitDataPool = new ScriptableObjectPool<UnitDataAAA>();
        public int count;
        public int spawned;
        public int despawned;
        public string status = "";
        public string current = "";
        public List<UnitDataAAA> unitDatas = new List<UnitDataAAA>();
    	void Start ()
        {
    ...
    //        StartCoroutine(TestLimit());
        ...
        public IEnumerator TestCull()
        {
            ...
            PoolManager.groups.common.CreatePool<UnitDataAAA>(unitDataPool);
            status = "Init";
            ...
            for(int j = 0; j < 10; j ++)
            {
                UnitDataAAA unitData = PoolManager.groups.common.Spawn<UnitDataAAA>();
            ...
            {
                UnitDataAAA unitData = unitDatas[j];
                PoolManager.groups.common.Despawn<UnitDataAAA>(unitData);
                Debug.Log(unitDataPool);
                status = "Despawn " + j;
                current =unitData != null ?  unitData.ToString() : "null";
                yield return new WaitForSeconds(1);
