...
namespace com.ihaiu
{
    public partial class PoolGroup 
    {
        ...
        private     List<string>                    _spawnedName        = new List<string>();
            ...
            for (int i = 0; i < this._prefabPools.Count; i++)
            {
                ...
                if (this._prefabPools[i].prefabGO.name == prefabName)
                {
                    ...
                    if (!despawn)
                    {
                        ...
                        if(isDebug) this._spawnedName.Add(instance.name);
                    }
        ...
        public Transform Spawn(Transform prefab, Vector3 pos, Quaternion rot, Transform parent)
        {
            ...
            for (int i = 0; i < this._prefabPools.Count; i++)
            {
                ...
                if (this._prefabPools[i].prefabGO == prefab.gameObject)
                {
                    ...
                    else if (inst.parent != prefabPool.parent)  // Auto organize?
                    {
                        ...
                        inst.SetParent(prefabPool.parent, false);
                    }

                  
                    this._spawned.Add(inst);
                    if(isDebug) this._spawnedName.Add(inst.name);
            ...
            {
                inst.SetParent(newPrefabPool.parent, false);
            }
        ...
        public void Despawn(Transform instance, string name)
        {
            if (instance == null)
            {
                Debug.LogError(string.Format("SpawnPool {0}: name={1} instance={2} not found in SpawnPool",
                this.groupName,
                name,
                instance));
                return;
...
//                            instance.name));
            ...
            if (isDebug) this._spawnedName.Remove(instance.name);
        }
        ...
        public void DespawnAll()
        {
            ...
            if (isDebug)
            {
                var spawnedName = new List<string>(this._spawnedName);
                for (int i = 0; i < spawned.Count; i++)
                {
                    this.Despawn(spawned[i], spawnedName[i]);
                }
            }
            else
            {
        ...
        public void DespawnParticle(Transform instance, float time = 0.5f)
        {
            StartCoroutine(OnDespawnParticle(instance, time));
        }



        private IEnumerator OnDespawnParticle(Transform instance, float time)
        {
            
            Renderer[] renderers;
            renderers =instance.GetComponents<Renderer>();
            ...
            particles =instance.GetComponents<ParticleSystem>();
            ...
            particles =instance.GetComponentsInChildren<ParticleSystem>();
            for(int i = 0; i < particles.Length; i ++)
            {
                particles[i].Clear();
                particles[i].Stop();
