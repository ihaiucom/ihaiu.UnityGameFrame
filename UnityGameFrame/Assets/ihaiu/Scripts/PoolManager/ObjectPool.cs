    ...
    public class ObjectPool<T> : AbstractObjectPool
    {
        ...
        */
        ...
        internal bool DespawnInstance(T xform, bool sendEventMessage)
        {
            ...
            if (this.logMessages)
                Debug.Log(string.Format("[对象池] {0} : 设置闲置对象 {1}, totalCount={2}",
                    this.name,
                    xform, totalCount));
        ...
        /** 验证实力对象有效性 */
        virtual public bool ValidityInstance(T inst)
        {
            return true;
        }
        ...
        {
            if (IsImplementIPoolItem && instance != null)
            {
