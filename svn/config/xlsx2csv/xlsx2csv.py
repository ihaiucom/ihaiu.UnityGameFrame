...
def convert_batch(path, kwargs, outputpath="", recursive=False):
    for name in os.listdir(path):
        fullpath = os.path.join(path, name)
        if os.path.isdir(fullpath):
            if recursive:
                convert_batch(fullpath, kwargs, outputpath, recursive)
        else:
            if fullpath.lower().endswith(".xlsx"):
                if outputpath == "":
                    outfilepath = fullpath[:-4] + 'csv'
                else:
                    outfilepath = os.path.join(outputpath, os.path.basename(fullpath)[:-4] + 'csv')
...
if __name__ == "__main__":
    ...
    parser.add_option("-i", "--ignoreempty", dest="skip_empty_lines", default=False,
                      action="store_true", help="skip empty lines")
    ...
    if options.batch:
        if len(args) >= 1:
            convert_batch(args[0], kwargs, outpath)
        else:
            parser.print_help()
    elif options.recursive:
        if len(args) >= 1:
            convert_batch(args[0], kwargs, outpath, True)
        else:
            parser.print_help()
    else:
        if len(args) < 1:
